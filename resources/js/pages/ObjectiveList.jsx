...
export default function ObjectiveList({
    ...
    const openConfirm = (title, message, onConfirm, confirmText = 'OK', cancelText = 'Hủy') => {
        setConfirmModal({
            show: true,
            title,
            message,
        ...
        if (showArchived) {
            const fetchArchived = async () => {
                ...
                try {
                    ...
                    const params = new URLSearchParams({ archived: "1", include_archived_kr: "1" });
    ...
    const reloadBothTabs = useCallback(async (token) => {
        ...
        if (showArchived) {
            const archivedParams = new URLSearchParams({ archived: "1", include_archived_kr: "1" });
            if (cycleFilter) archivedParams.append("cycle_id", cycleFilter);
            const archivedRes = await fetch(`/my-objectives?${archivedParams}`, {
                headers: { Accept: "application/json", "X-CSRF-TOKEN": token },
            });
    ...
    const handleArchiveKR = async (krId) => {
        openConfirm(
            ...
            async () => {
                ...
                try {
                    const token = document.querySelector('meta[name="csrf-token"]')?.getAttribute("content");
                    const obj = items.find(o => o.key_results?.some(kr => kr.kr_id === krId));
                    if (!obj) throw new Error("Không tìm thấy OKR cha.");

                    const res = await fetch(`/my-key-results/${obj.objective_id}/${krId}/archive`, {
                        method: 'POST',
                        headers: { 'X-CSRF-TOKEN': token, 'Accept': 'application/json' },
                    });
                    const json = await res.json();
                    if (!json.success) throw new Error(json.message || 'Lưu trữ thất bại');
    ...
    const handleUnarchiveKR = async (krId) => {
        openConfirm(
            ...
            async () => {
                ...
                if (!obj) {
                    setToast({ type: "error", message: "Không tìm thấy OKR cha." });
                ...
                try {
                    ...
                    const json = await res.json();
                    if (!json.success) throw new Error(json.message || 'Bỏ lưu trữ thất bại');
        ...
        openConfirm(
            ...
            async () => {
                ...
                try {
                    ...
                    const res = await fetch(`/my-key-results/destroy/${obj.objective_id}/${krId}`, {
                        method: 'DELETE',
                        headers: { 'X-CSRF-TOKEN': token, 'Accept': 'application/json' },
                    });
                    const json = await res.json();
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody className="divide-y divide-slate-100">
                                                ...
                                                {obj.key_results && obj.key_results.length > 0 && (
                                                    ...
                                                    >
                                                        <svg
                                                            ...
                                                            viewBox="0 0 20 20"
                                                            fill="currentColor"
                                                            className={`w-4 h-4 text-slate-500 group-hover:text-slate-700 transition-transform duration-200 ${openObj[obj.objective_id] ? 'rotate-90' : ''}`}
                                                        >
                        ...
                        {showArchived && !loadingArchived && archivedItems.length === 0 && (
                            <tr>
                                <td colSpan={9} className="px-3 py-5 text-center text-slate-500">Không có OKR nào Lưu trữ.</td>
                            </tr>
                        )}
                        ...
                        {showArchived && !loadingArchived && (
                            <>
                                ...
                                {archivedItems
                                    ...
                                    .map((obj, index) => (
                                        <React.Fragment key={obj.objective_id}>
                                                            ...
                                                            {obj.key_results?.some(kr => kr.archived_at) && (
                                                                ...
                                                                >
                                                                    <svg
                                                                        ...
                                                                        viewBox="0 0 20 20"
                                                                        fill="currentColor"
                                                                        className={`w-3.5 h-3.5 text-gray-400 group-hover:text-gray-600 transition-all duration-300 ${(openObj[obj.objective_id] ?? false) ? 'rotate-90' : 'rotate-0'}`}
                                                                    >
                                                                        <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
                                                                    </svg>
                                                                </button>
                                            ...
                                            {(openObj[obj.objective_id] ?? false) && obj.key_results?.filter(kr => kr.archived_at)?.map(kr => (
                                                <tr key={kr.kr_id} className="bg-gray-50">
                                                    <td className="px-8 py-2 italic text-gray-600">{kr.kr_title}</td>
                                                    <td colSpan={7}></td>
                                                    <td className="text-center py-2">
                                                        <div className="flex items-center justify-center gap-2">
                                                            <button
                                                                onClick={() => handleUnarchiveKR(kr.kr_id)}
                                                                disabled={unarchivingKR === kr.kr_id}
                                ...
                                {archivedItems
                                    .filter(obj => obj.archived_at === null && obj.key_results?.some(kr => kr.archived_at))
                                    .map((obj, index) => {
                                        const archivedKRs = obj.key_results.filter(kr => kr.archived_at);
                                        const hasArchivedKRs = archivedKRs.length > 0;

                                        return (
                                            <React.Fragment key={`archived-kr-${obj.objective_id}`}>
                                                <tr className={`bg-gradient-to-r from-yellow-50 to-orange-50 border-t-2 border-yellow-300 ${index > 0 ? "mt-4" : ""}`}>
                                                    <td className="px-3 py-3 border-r border-slate-200">
                                                        <div className="flex items-center justify-between w-full">
                                                            <div className="flex items-center gap-1">
                                                                {hasArchivedKRs && (
                                                                    <button
                                                                        onClick={() => setOpenObj(prev => ({
                                                                            ...prev,
                                                                            [obj.objective_id]: !(prev[obj.objective_id] ?? false)
                                                                        }))}
                                                                    ...
                                                                    >
                                                                        <svg
                                                                            ...
                                                                            viewBox="0 0 20 20"
                                                                            fill="currentColor"
                                                                            className={`w-3.5 h-3.5 text-orange-500 group-hover:text-orange-700 transition-all duration-300 ${(openObj[obj.objective_id] ?? false) ? 'rotate-90' : 'rotate-0'}`}
                                                                        >
                                                                            <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
                                                                        </svg>
                                                                    </button>
                                                ...
                                                {openObj[obj.objective_id] && archivedKRs.map(kr => (
                                                    <tr key={kr.kr_id} className="bg-gray-50">
                                                        <td className="px-8 py-2 italic text-gray-600">
                                                            {kr.kr_title} <span className="text-orange-600 text-xs"></span>
                                                        </td>
                                                        <td colSpan={7}></td>
                                                        <td className="text-center py-2">
                                                            <div className="flex items-center justify-center gap-2">
