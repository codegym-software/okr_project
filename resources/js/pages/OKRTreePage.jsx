...
export default function OKRTreePage() {
            ...
            try {
                ...
                if (json.success) {
                    ...
                    if (json.data && json.data.length > 0) {
                        const currentSelected = json.data.find(
            ...
            try {
                ...
                if (json.success) {
                    ...
                    setToast({
                        type: "error",
                        message:
                            json.message || "Không thể tải dữ liệu tree view",
                    });
                }
            } catch (error) {
                setToast({
    ...
    const filterNodesAndEdges = useCallback(
        (allNodesList, allEdgesList, expandedSet) => {
            ...
            const rootNodes = allNodesList.filter(
                (node) => !allEdgesList.some((edge) => edge.target === node.id)
            );
    ...
    useEffect(() => {
        ...
        const buildFlowData = (node, parentId = null) => {
            const nodeId = node.objective_id
                ? `obj-${node.objective_id}`
                : `kr-${node.kr_id}`;
            const nodeType =
                node.type === "key_result" || node.kr_id
                    ? "key_result"
                    : "objective";
        ...
        const rootNodes = flowData.nodes.filter(
