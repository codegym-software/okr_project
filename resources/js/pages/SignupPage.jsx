...
export default function SignupPage() {
    ...
    useEffect(() => {
        ...
        style.textContent = `
            ...
            input#password_confirmation::-webkit-strong-password-auto-fill-button {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setErrors({ email: [response.data.message || "Đăng ký thất bại."] });
            }
        } catch (error) {
            ...
            if (error.response) {
                ...
                if (error.response.data?.errors) {
                    ...
                    // Map password.confirmed errors to password_confirmation field
                    if (serverErrors.password && Array.isArray(serverErrors.password)) {
                        const passwordErrors = serverErrors.password;
                        const confirmedError = passwordErrors.find(err => 
                            typeof err === 'string' && err.includes('khớp')
                        );
                        if (confirmedError) {
                            // Move confirmed error to password_confirmation
                            if (!serverErrors.password_confirmation) {
                                serverErrors.password_confirmation = [];
                            }
                            serverErrors.password_confirmation.push(confirmedError);
                            // Remove from password errors
                            serverErrors.password = passwordErrors.filter(err => err !== confirmedError);
                        }
                    }
                    setErrors(serverErrors);
                } else if (error.response.data?.message) {
                    // Check if message is about password
            ...
            <div className="max-w-md w-full">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm text-gray-600">
                        Tạo tài khoản mới để bắt đầu sử dụng OKRun.
                    </p>
                </div>
