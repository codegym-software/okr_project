...
const ToastNotification = ({
    ...
    }, [isVisible, duration, onClose, resolvedMessage]);
    ...
    const colorClass =
        resolvedType === "success" ? "bg-emerald-600" : "bg-red-600";

    return (
        <div className="fixed top-4 right-4 z-[9999] animate-in slide-in-from-top-5 duration-300">
            <div
                className={`flex items-center gap-3 px-5 py-3 rounded-xl shadow-lg text-white font-medium text-sm transition-all ${colorClass}`}
            >
                <svg
                    className="w-5 h-5"
                    fill="none"
